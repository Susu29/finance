...
def index():
    ...
    user_transactions = db.execute("SELECT symbol, SUM(shares) AS total_shares FROM orders WHERE user_id = ? GROUP BY symbol HAVING SUM(shares) > 0", session["user_id"])
    ...
    for line in user_transactions:
        ...
        # Multiply
        line["actual_total_value"] = round(line["actual_value"] * line["total_shares"], 2)
...
def buy():
    if request.method == "POST":
        ...
        if not request.form.get("symbol"):
            return apology("Please enter a smymbol", 400)
        if not request.form.get("shares"):
            return apology("Please enter share amount", 400)
        try:
            shares = int(request.form.get("shares"))
        except:
            return apology("Nice try, but there is server side confirm")
        if shares <= 0:
